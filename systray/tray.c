...
static pixman_image_t *createcanvas(int width, int height, int bgcolor) {
  ...
  pixman_image_composite32(PIXMAN_OP_SRC, src, NULL, dest, 0, 0, 0, 0, 0, 0,
                           width, height);
  ...
  wl_list_for_each(item, &watcher->items, link) {
    ...
    if (item->icon) {
      ...
      if (resize_image(img, icon_size, icon_size) < 0)
        goto fail;
      pixman_image_composite32(PIXMAN_OP_OVER, img, NULL, canvas, 0, 0, 0, 0,
                               slot_x_start, 0, canvas_width, canvas_height);
      ...
      pixman_image_composite32(PIXMAN_OP_OVER, fg, g->pix, canvas, 0, 0, 0, 0,
                               pen_x, pen_y, canvas_width, canvas_height);
...
void tray_rightclicked(Tray *tray, unsigned int index, const char **menucmd) {
  ...
  wl_list_for_each(item, &watcher->items, link) {
    if (count == index) {
      menu_show(watcher->conn, watcher->loop, item->busname, item->menu_busobj,
                menucmd);
      return;
    }
    count++;
